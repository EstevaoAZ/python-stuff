...
2 - As tuplas são imutáveis: Isso ignifica que ao criar uma tupla, ela não muda. Toda
...
# Podemos gerar uma tupla dinamicamete com range(inicio, fim, passo)
...
# OBS: Gera erro (value error) se colocarmos um número diferente de elementos para desempacotar.

# Métodos para: adição e remoção de elementos nas tuplas não existem, dado o fato das tulas serem imutáveis.
...
# O acess a elementos de uma tupla também é semelhante a de uma lista
...
nova = nova + outra # Na tupla não temos problema de Shallow Copy
